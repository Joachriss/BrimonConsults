...
export const ProjectCard = ({  cols,project }: {  cols: number | undefined, project: any }) => {
    ...
    return (
        <Link to={`/projects/${project.title}`} className={`col-span-${cols} h-56 overflow-hidden duration-200 group relative hover:scale-[101%]`}>
            {   Array.isArray(images) &&
                images.map((image,index) => (
                    <div className={`absolute inset-0 transition-transform  duration-500 w-full ${index === currentIndex ? 'translate-x-0' : 'translate-x-full'}`} key={index}>
                        <img className="w-full h-full mb-4 rounded-lg object-cover" src={image} alt="project image " />
                    </div>
                ))
            }
            <div className="absolute bottom-0 w-full bg-linear-to-t from-black to-transparent h-3/4 pt-22 flex  text-white p-3  flex-col">
                <h1 className="font-bold text-xs md:text-lg line-clamp-1 flex gap-x-1"><ImLocation2 size={20}/><div>{project.title}</div> </h1>
