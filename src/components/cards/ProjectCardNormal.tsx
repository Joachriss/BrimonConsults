...
export const ProjectCardNormal = ({project,index}:TProps) => {
    return (
        ...
        >
            <Link to={`/projects/${project.title}`} rel="noopener noreferrer">
                <div className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl easy-in-out transition-shadow duration-300">
                    <img src={project?.images[0] as string} alt={project.title} className="w-full ease-in-out duration-300 h-48 object-cover group-hover:scale-105" />
                    <div className="py-4">
                        <hr className='w-3/4 group-hover:w-full duration-300 my-1 border-4 border-[#d94766]'/>
                        <h3 className=" px-4 text-xl font-semibold text-[#194062]">{project.title}</h3>
